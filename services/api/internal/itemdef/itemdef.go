...
// Definition is one Item Definition master entry.
...
func (c *Catalog) Seed(ctx context.Context, st *store.Store) error {
	for _, d := range c.list {
		...
		if err := st.UpsertItemDefinition(ctx, d.ItemDefinitionID, d.Tags, d.StackLimit, d.WeightMilli, d.Rarity, d.BaseValue, useEffect); err != nil {
			return err
		}
	}
	return nil
